...
def main_loop(target, open_camera_frame=False):
    s1 = Servo(12, base=BASE_X)
    s2 = Servo(13, base=BASE_Y)

    camera = Camera(show_windows=open_camera_frame)
    pid_x = create_pid(target[0])
    pid_y = create_pid(target[1], Ku=0.130)
    ...
    while True:
        ...
        if orange is None:
            ...
            print(f'\tDesired = {target}\tActual = ({x}, {y})')
...
if __name__ == '__main__':
    main_loop(target=center, open_camera_frame=False)
